...
En esta sección se presenta formalmente el framework que se utilizará durante este trabajo. \emph{PredictionIO} es un servidor de \emph{Machine Learning} de código abierto para Científico de Datos y Desarrolladores que permite crear motores de predicción para ambientes de producción, con un bajo tiempo de entrenamiento y despliegue en ambientes productivos. Principalmente está construido en \emph{Apache Spark, HBase} y \emph{Spray}. 

Como ya se ha señalado este ambiente de trabajo se encuentra en un maduración estable y constante que permite tanto disponer servidores con motores predictivos, como también toda una infraestructura distribuida para hacer que complejos algoritmos sean utilizados para solucionar problemas reales.
...
\subsection{Despliegue de motor de predicción}
  ...
  Un \emph{Engine} pone todos los componentes \emph{DASE} en un estado especifico de despliegue
  \begin{enumerate}
