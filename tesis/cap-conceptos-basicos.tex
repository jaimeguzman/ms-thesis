...
Un alfabeto es un conjunto ordenado de elementos. Una de sus características es que la cantidad de elementos puede ser de valores  discreto, representaremos por $\Sigma$ al alfabeto que usaremos y una de las características básica de este alfabeto posee $\sigma^{n}$ símbolos, tal que $\sigma^{n}= \sigma_{1}, \dots, \sigma_{n}, \sigma_{i} \in \Sigma$, donde $\sigma_{1}$, puede ser definido como la página inicial de una cierta secuencia  o sesión de usuario. Adicionalmente daremos como restricción que $|\Sigma| >= 2$ símbolos necesarios para realizar experimentos acotados de longitud de secuencia o la cardinalidad del alfabeto. Posteriormente en el capítulo experimental\ref{ch:experimetal-all} usaremos un conjunto de datos de tamaño $|\Sigma| =\ 17\  \mbox{símbolos}$, para datos reales recolectados de \emph{MSNBC}\cite{Claude2014} y un $|\Sigma|$ variable para experimentos con datos sintéticos.
